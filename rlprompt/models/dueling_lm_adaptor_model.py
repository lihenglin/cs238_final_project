...
class DuelingLMAdaptorModel(BaseModel):
    ...
    def __init__(
        ...
        self.mlp_v = _build_one_layer_mlp(in_dim=model_dim,
                                          out_dim=model_dim,
                                          hidden_size=hidden_size).to(self.device)
        self.mlp_a = _build_one_layer_mlp(in_dim=model_dim,
                                          out_dim=model_dim,
                                          hidden_size=hidden_size).to(self.device)
        ...
        self.mlp_v.apply(_init_weights)
        self.mlp_a.apply(_init_weights)

    def _mlp_forward(self, state: torch.Tensor) -> torch.Tensor:
        value = self.generator.model.lm_head(self.mlp_v(state)).logsumexp(dim=-1, keepdim=True)
        raw_advantage = self.generator.model.lm_head(self.mlp_a(state))
        advantage = raw_advantage - raw_advantage.logsumexp(dim=-1, keepdim=True)  
